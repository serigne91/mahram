...
<img src='IURL/image.gif' alt='Image' class=vdev onclick='selectImage()'>\
...
function doFormatF(arr)
{
  ...
  else if(cmd[0]=='Image') editImage(cmd[1],el);
...
{ 
  ...
  var arr=showModalDialog(urlx, arg, "font-family:Verdana;font-size:12;dialogWidth:30em;dialogHeight:40em; edge:sunken;help:no;status:no");
  if(arr !=null) doFormatF(cmd+','+arr)
}

function doFormatUpload(file,cmd,arg)
{ 
  var urlx= QBPATH + '/' + file
  ...
  var arr=showModalDialog(urlx, arg, "font-family:Verdana;font-size:12;dialogWidth:32em;dialogHeight:40em; edge:sunken;help:no;status:no");
  if(arr !=null) doFormatF(cmd+','+arr)
}
...
{ 
  doFormatUpload('image2.php3','InsertImage',QBPATH)
}
function selectImage()
{ 
  doFormatDialog('image.php3','InsertImage',QBPATH)
}
...
function doMUp()
{
 ...
 if(typeof(ASKED)=="undefined" && el.type=='textarea' && VISUAL==0)
  { visual=confirm("Utiliser le mode visuel ?"); if(!visual) ASKED=1; }
